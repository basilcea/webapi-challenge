...
`
 class Form extends React.Component{
     ...
     getFriend = async (id) =>{
        ...
        const newProject = this.props.data.find(friend => friend.id === Number(id))
        this.setState({
            name: newProject.name,
            bio: newProject.description,
            value: "Update",
            target: id,
        })
        }
   ...
   render(){
    return (
        ...
        <Formed onSubmit ={(e) =>this.props.submit(e , this.state.target)}>
            ...
            <input type='text' onChange={(e)=>this.props.inputValue(e.target.value, 'description')} placeholder={this.state.description} />
